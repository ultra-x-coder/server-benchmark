...
見やすいターミナルレポートを表示し、JSON レポートを保存し、2 台のサーバーを比較できます。
**既定では何もインストールしません**（`sudo` なし・確認なし・不足ツールの指標は単にスキップ）ので、**本番サーバーでも安全に実行できる**よう設計されています。
不足パッケージのインストールを許可するには `--install` でオプトインしてください。
...
./benchx.sh --no-install          # 既定の挙動：既存のツールのみで実行（インストール・確認なし）
./benchx.sh --install             # 不足パッケージのインストールを許可（まず警告を表示・sudo が必要・各パッケージごとに確認）
./benchx.sh --install --yes       # 各パッケージの確認なしですべてインストール（非対話実行）
...
| `--no-install` | **既定の挙動**：既存のツールのみで実行（インストール・sudo・確認なし） |
| `--install` | 不足パッケージのインストールをオプトインで許可：まず目立つ警告を表示し、`sudo` で root を要求し、**各パッケージごとに個別の確認を求める**（インストールするものを選び、残りをスキップできる）。`--yes` を併用するとこれらの確認を飛ばしてすべてインストール |
| `--reinstall` | 必要パッケージを強制再インストール（Ctrl-C 後に壊れた dpkg も修復） |
| `--confirm-each` | 各パッケージのインストール/再インストール前に確認（`--install` はこれを既定で含む） |
...
**既定では、スクリプトは何もインストールしません。** 単純な `./benchx.sh` 実行は `sudo` を使わず・確認も求めず、不足ツールの指標は丁寧にスキップ・記録します。これにより本番サーバーでも既定実行が安全になります。

> **TIP** `--install` なしで実行すると（本番安全な既定の no-install モード）、出力の冒頭付近に案内バナーが表示されます。これは no-install モードで実行中のため一部の指標がスキップされる可能性があること、ツールが無いとまっさらなサーバーでは redis / nodejs / nginx / mongodb などのアプリ指標や speedtest が利用できない場合があることを説明します。不足ツールをインストールして**完全な結果**を収集するには `--install` を付けて再実行してください（まず警告を表示・`sudo` が必要）。さらに `-y` を付けると、各パッケージごとの確認なしですべてをインストールします： `./benchx.sh --install -y`。このバナーは `--install` または `--safe` を使用した場合には表示されません。

不足パッケージのインストールを許可するには **`--install`** を使ってください。指定すると、スクリプトはパッケージマネージャ（Linux の `apt`/`dnf`/`yum`/`pacman`/`zypper`/`apk`、macOS の `brew`）を自動検出し、不足分をインストールできます。

- `--install` を指定すると、スクリプトはまず**大きく目立つ赤い警告**を表示します —— パッケージの（再）インストールは稼働中のサーバーを壊す可能性があります（`/etc` 設定の上書き、redis/nginx/mongodb などサービスの再起動・停止、挙動が変わるアップグレードの取り込み）。
- 続いて、Linux ではシステムパッケージのインストールに **root** が必要なため、`sudo` 使用の許可を **一度だけ** 尋ねます。さらに、**各パッケージごとに個別の確認を求める**ので、インストールするものを選び、残りをスキップできます（`--confirm-each` と同じ挙動で、`--install` はこれを既定で含みます）。`--yes` を併用すると、これらの確認を飛ばしてすべてをインストールします（非対話実行向け）。
- `--install` を指定しない場合（既定）、または `--no-install`/`--safe` の場合は、**root なし** で使えるものだけを使い、それ以外は丁寧にスキップ・記録します。
...
- `--reinstall` は壊れた `dpkg` 状態（例: 中断された `apt` の後）を修復し、パッケージを強制再インストールします。
  ...
  データは削除しませんが、本番サーバーでは既定（インストールなし）のまま、または `--safe` を推奨します。
