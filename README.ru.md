...
удобно сравнивать между серверами, чтобы понять «насколько сервер A быстрее сервера B под redis». Пометка
`≈` означает, что индекс — оценка только по синтетике (реальный бенчмарк движка не выполнялся).
