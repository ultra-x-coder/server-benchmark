...
В этом режиме (без `--install`) скрипт печатает вверху вывода информационный баннер **TIP**: он напоминает,
что прогон идёт в режиме no-install, поэтому часть метрик может быть пропущена; из-за отсутствующих
инструментов на «голом» сервере могут быть недоступны app-метрики (`redis` / `nodejs` / `nginx` / `mongodb`)
и speedtest. Повторный запуск с `--install` доставит недостающие инструменты и соберёт **полный** набор
результатов (сначала с предупреждением; нужен `sudo`), а добавление `-y` поставит всё без вопросов по каждому
пакету: `./benchx.sh --install -y`. Баннер **не** показывается при `--install` или `--safe`.
