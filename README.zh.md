...
- 不使用 `--install`（即默认，或显式 `--no-install`/`--safe`）时，只使用已存在的工具，其余优雅跳过并注明。
  在该 **no-install** 模式下（即未传 `--install`），脚本会在输出顶部附近打印一条信息性的 **TIP** 提示横幅，说明：
  - 当前运行于 no-install 模式，因此部分指标可能被跳过；
  - 缺少工具意味着在裸服务器上，redis / nodejs / nginx / mongodb 等应用指标以及 speedtest 可能不可用；
  - 以 `--install` 重新运行可安装缺失的工具并收集**完整**结果（会先警告，且需要 `sudo`）；
  - 加上 `-y` 可安装全部而不逐个软件包询问：`./benchx.sh --install -y`。

  使用 `--install` 或 `--safe` 时，该横幅不会出现。
